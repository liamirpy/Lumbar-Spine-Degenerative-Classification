        ...
        for dcm in list_of_dcm_file:
            ...
            if len(boxes) !=0:
                ...
                for box in boxes:
                    ...
                    new_result = {
                    'condition': classes_dictionary[belong_to_class],
                    'confident_of_condition': confidential,
                    'score_prediction': list(predictions)
                        }


                # Check if the condition already exists in the prediction_result
                    existing_entry = next((item for item in prediction_result if item['condition'] == new_result['condition']), None)
                    
                    if existing_entry:
                        # If the condition exists, check if the new confidence is greater than the existing confidence
                        if new_result['confident_of_condition'] > existing_entry['confident_of_condition']:
                            # Update the existing entry with the new result
                            existing_entry.update(new_result)
                    else:
                        # If the condition doesn't exist, append the new result
                        prediction_result.append(new_result)
        ...
        for study_id in list(study_ids_report.keys()):
            ...
            for condition in all_conditions:
                ...
                if len(all_score_predicted_for_this_condition)==0:
                    ...
                    study_ids_report[study_id][condition]=[0.5,0.5,0.5]    

                else:


                    averages_list = [sum(x) / len(x) for x in zip(*all_score_predicted_for_this_condition[0])]
...
# newwwwwww {'condition': 'Spinal_Canal_Stenosis_L1_L2', 'confident_of_condition': 0.2614451050758362, 'score_prediction': [array([      0.806,     0.18012,    0.013876], dtype=float32)]}


# prrrrr [{'condition': 'Spinal_Canal_Stenosis_L4_L5', 'confident_of_condition': 0.42469388246536255, 'score_prediction': [array([   0.035285,     0.34921,     0.61551], dtype=float32)]},
#  {'condition': 'Spinal_Canal_Stenosis_L3_L4', 'confident_of_condition': 0.375446617603302, 'score_prediction': [array([    0.81695,      0.1638,    0.019253], dtype=float32)]}, 
#  {'condition': 'Spinal_Canal_Stenosis_L1_L2', 'confident_of_condition': 0.3923245370388031, 'score_prediction': [array([    0.39757,     0.49234,     0.11009], dtype=float32)]},
#   {'condition': 'Spinal_Canal_Stenosis_L5_S1', 'confident_of_condition': 0.4563785195350647, 'score_prediction': [array([    0.35786,     0.50576,     0.13638], dtype=float32)]}, 
#   {'condition': 'Spinal_Canal_Stenosis_L2_L3', 'confident_of_condition': 0.3390381634235382, 'score_prediction': [array([    0.25183,     0.38235,     0.36582], dtype=float32)]}]
